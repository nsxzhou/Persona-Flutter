...
4. 标题层级、章节顺序必须严格遵守要求，不要缺节，不要重排。
5. 分析结论必须最终能转化为可执行写作资产：句法、节奏、词汇、标点、对白、意象和叙述视角必须尽量落到操作规则。
''';
...
''';
  ...
  String buildMergePrompt({required List<String> chunkAnalyses}) {
    return '''
$_sharedRules

你正在执行全局聚合阶段。请把多个 chunk 的 Markdown 分析合并成一份统一的 Markdown 报告草稿。
要求：同义归并、重复证据去重、弱判断保留，保持章节顺序。
...
''';
  ...
  String buildReportPrompt({required String mergedAnalysisMarkdown}) {
    return '''
$_sharedRules
